...
#     if user_in.email:
#         existing_user = crud.get_user_by_email(session=session,
#                                                email=user_in.email)
...
# def update_password_me(*, session: SessionDep, body: UpdatePassword,
#                        current_user: CurrentUser) -> Any:
#     """
#     Update own password.
#     """
...
def create_user_open(session: SessionDep, user_in: UserCreateOpen) -> Any:
    ...
    user = crud.get_user_by_phone(session=session, phone=user_in.phone)
    if user:
        raise HTTPException(
            status_code=400,
            detail="The user with this email already exists in the system",
...
#     db_user = crud.update_user(session=session,
#                                db_user=db_user,
#                                user_in=user_in)
#     return db_user
